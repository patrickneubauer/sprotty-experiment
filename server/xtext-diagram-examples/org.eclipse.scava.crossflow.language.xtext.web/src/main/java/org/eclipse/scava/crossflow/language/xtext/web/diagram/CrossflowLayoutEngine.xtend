...
class CrossflowLayoutEngine extends ElkLayoutEngine {
	
	static val LOG = Logger.getLogger(CrossflowLayoutEngine)
	
	override layout(SModelRoot root) {
		if (root instanceof SGraph) {
			val configurator = new SprottyLayoutConfigurator
			configurator.configureByType('crossflow')
...
//	}
	...
	override protected resolveReferences(ElkEdge elkEdge, SEdge sedge, Map<String, ElkConnectableShape> id2NodeMap, LayoutContext context) {
		...
		if (source instanceof ElkNode) {
			val index = sedge.sourceId//if (sedge instanceof FlowEdge) sedge.sourceIndex
			if (index === null)
...
//				port.setProperty(CoreOptions.PORT_INDEX, index)
				port.setProperty(CoreOptions.PORT_INDEX, Integer.valueOf(index))
				elkEdge.sources.add(port)
			}
		}
		val target = id2NodeMap.get(sedge.targetId)
		if (target instanceof ElkNode) {
			val index = sedge.targetId//if (sedge instanceof FlowEdge) sedge.targetIndex
			if (index === null)
