 ...
 */
class CrossflowServerLauncher {
	
	static val LOG = Logger.getLogger(CrossflowServerLauncher)
	...
	def static void main(String[] args) {
		try {
			...
			val injector = new CrossflowWebSetup().createInjectorAndDoEMFRegistration()
			val launcher = injector.getInstance(CrossflowServerLauncher)
			launcher.launch()
		} catch (Exception e) {
			System.err.println("Exception occurred in CrossflowServerLauncher main method: " + e.getMessage())
		}
	}
...
//				'.*/io\\.typefox\\.sprotty\\.example\\.multicore\\.web/.*,' +
				'.*/org\\.eclipse\\.scava\\.crossflow\\.language\\.xtext\\.web/.*,' +
		...
		val log = new Slf4jLog(CrossflowServerLauncher.name)
		try {
			server.start
