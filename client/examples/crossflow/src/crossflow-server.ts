...
class SelectionHandler implements IActionHandler {
    handle(action: SelectAction): void {
        ...
        if (xtextService !== undefined) {
            const selectedElement = action.selectedElementsIDs.length > 0 ? action.selectedElementsIDs[0] : 'crossflow';
            xtextService.select({
...
export function setupCrossflow(websocket: WebSocket) {
    ...
    websocket.addEventListener('open', event => {
        ...
        function run() {
            ...
            if (xtextServices !== undefined) {
                ...
                diagramServer.clientId = resourceId + '_crossflow';
...
export default function runCrossflowServer() {
    const websocket = new WebSocket('ws://' + window.location.host + '/diagram');
    // const websocket = new WebSocket('ws://' + '0.0.0.0' + '/diagram');
    setupCrossflow(websocket);
}
